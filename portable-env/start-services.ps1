...
if (-not $isPgRunning) {
    ...
    # ใช้ Start-Process แบบไม่รอ (ไม่ใช้ -Wait) เพื่อป้องกันการค้าง
    Start-Process -FilePath $pgCtl -ArgumentList "start -D `"$dataDir`" -o `"-p 5433`"" -NoNewWindow 
    
    # รอเพื่อให้ PostgreSQL เริ่มต้น
    Write-Host "รอ PostgreSQL เริ่มต้น..." -ForegroundColor Yellow
